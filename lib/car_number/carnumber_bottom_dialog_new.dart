...
openCarNumberViewDialogNew(
  ...
  showModalBottomSheet(
      ...
      builder: (w) {
        return CarNumberKeyViewNew(carNumber, submit);
      });
}

///车牌号数据框+车牌号键盘-底部弹框
class CarNumberKeyViewNew extends StatefulWidget {
  const CarNumberKeyViewNew(this.car_number, this.submit, {Key? key})
      : super(key: key);
...
class _CarNumberKeyViewNewState extends State<CarNumberKeyViewNew> {
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  "编辑车牌",
                  style: TextStyle(color: color_00131D, fontSize: (16)),
                ),
                InkWell(
                  onTap: StringUtils.stringListToString(carNumber).length > 6
                      ? () {
                          mCarNumber.value =
                              StringUtils.stringListToString(carNumber);
                          widget.submit!(
                              StringUtils.stringListToString(carNumber));
                          Navigator.pop(context);
                        }
                  ...
                  child: Container(
                      ...
                      child: Text(
                        "确定",
                        style: TextStyle(
                            color: StringUtils.stringListToString(carNumber)
                                        .length >
                                    6
                                ? color_00131D
                                : color_8F9296,
          ...
          Expanded(
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  getCarNumberKeyView(isInputABC
                      ? carnumber_numbers
                      : carnumber_provinces_list[0]),
                  getCarNumberKeyView(isInputABC
                      ? carnumber_alphabets[0]
                      : carnumber_provinces_list[1]),
                  getCarNumberKeyView(isInputABC
                      ? carnumber_alphabets[1]
                      : carnumber_provinces_list[2]),
                  getCarNumberKeyView(
                      isInputABC
                          ? carnumber_alphabets[2]
                          : carnumber_provinces_list[3],
                      isEnd: true),
                ],
              ),
            ),
  ...
  List<Widget> getCarNumberInputView() {
    ...
    for (int i = 0; i < maxCarNumberLength; i++) {
      if (carNumbers.length > i) {
        views.add(carNumberInputView(carNumbers[i],
            index: i,
            borderSelect: currentIndex == i,
            marginLeft: i == 0
                ? 0
                : i == 2
                    ? 24
  ...
  Widget carNumberInputView(String? text,
      {borderSelect = false, double marginLeft = 8, index = -1}) {
    return InkWell(
      onTap: () {
        if (index != -1) {
          setState(() {
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(
            color: borderSelect ? color_00131D : color_DADBDC,
            width: (1),
          ),
        ...
        child: Text(
          text ?? '',
          style: TextStyle(
              color: color_00131D, fontSize: (18), fontWeight: FontWeight.w700),
        ),
      ),
    );
  }
  ...
  Widget getCarNumberKeyView(List<String> datas, {bool isEnd = false}) {
    ...
    if (isEnd)
      views.add(Expanded(
          child: CarNumberKeyView(isInputABC ? mProvince : mABC,
              backColor: color_DADBDC)));
    datas.forEach((e) {
      views.add(CarNumberKeyView(e));
    });
    if (isEnd)
      views.add(
          Expanded(child: CarNumberKeyView(mDelete, backColor: color_DADBDC)));
          ...
          child: Container(
            ...
            child: text == mDelete
                ? Text(
                    '删除',
                    style: TextStyle(
                        color: text == 'I' ? Color(0XFFB7BABC) : color_00131D,
                        fontSize: (18)),
                  )
                : Text(
                    text,
